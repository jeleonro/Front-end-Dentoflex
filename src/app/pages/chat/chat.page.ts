...
import { sendOutline, arrowBackOutline, personCircleOutline, chatbubbleOutline } from 'ionicons/icons';
import {
  IonContent, IonIcon, IonButton, IonFooter, IonSpinner
...
export class ChatPage implements OnInit, OnDestroy, AfterViewChecked {
  ...
  constructor(
    ...
    addIcons({arrowBackOutline,personCircleOutline,chatbubbleOutline,sendOutline});
  }
  ...
  enviar() {
    ...
    this.chatService.enviarMensaje(this.citaId, texto).subscribe({
      ...
      error: () => { this.enviando = false; this.mensaje = texto; },
    });
  }
